...
function placeCard(p){return `<article class="card place-card" data-category="${esc(p.category)}">${imageTag(p,p.name)}<div class="section-head"><div><div class="eyebrow">${esc(p.category)}</div><h3>${esc(p.name)}</h3></div></div><p class="muted">${esc(p.description.slice(0,155))}…</p><a class="button secondary" href="#/luogo/${p.id}">Dettagli</a></article>`}
...
function foodCard(r){return `<article class="card food-card" data-food-tags="${esc((r.tags||[]).join(' '))}">${imageTag(r,r.name)}<div class="section-head"><div><div class="eyebrow">${esc(r.cuisine)}</div><h3>${esc(r.name)}</h3></div></div>${foodBadges(r)}<p class="muted">${esc(r.whyGo||r.description)}</p><a class="button secondary" href="#/ristorante/${r.id}">Dettagli</a></article>`}
